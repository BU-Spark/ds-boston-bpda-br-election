...
def read_data_post2006(filename):
    """
    :function: reads in csv file and returns dataframe 
    :params: filename - filepath for dataset that is 2010 or after.
    :returns: dataframe
    """
...
def read_data_pre2010(filename):
    """
    :function: reads in csv file and returns dataframe 
    :params: filename - filepath for dataset that is before 2010
    :returns:dataframe
    """
    data = pd.read_csv(filename)
    data = pd.DataFrame(data)
...
def clean2018(df):
    """
    :function: the 2018 data treats the characters with accents incorrectly, and thus doesn't match
               the data of the rest. this function will fix it so the accents are treated correctly.
    :param: df - A pandas dataframe containing the 2018 Electorate data
    """
...
def add_education_codes(data_2002, data):
    """
    :function: Adds a column to the dataframe for "data" called educational attainment codes, using the
    codes from 2002.
    :params:data_2002 - data from electorate in 2002, dataframe
    ...
    #Create list of educationl attainment codes, where each entry is a code for a row of the data
    column_codes = []
